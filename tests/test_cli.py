...
def test_cli_writes_run_manifest(tmp_path, monkeypatch, data_dir):
    import json
    from portfolio_extract.models import (ExtractionRecord, MetricName, CanonicalUnit, Currency,
        ExtractionMethod, ConfidenceTier, AbsenceReason, PeriodBasis, Sector)
    from portfolio_extract.pipeline import DocumentExtraction
    ...
    rec = ExtractionRecord(company="NovaCloud", period_year=2025, period_quarter="Q2",
        metric=MetricName.REVENUE_QUARTERLY, value=8.4, canonical_unit=CanonicalUnit.USD_MILLIONS,
        currency=Currency.USD, raw_text="$8.4M", label_as_reported="Revenue", source_file="NovaCloud_Q2_2025.pdf",
        ...
        period_basis=PeriodBasis.FLOW_QUARTERLY)
    de = DocumentExtraction(records=[rec], company=CompanyRecord(canonical_name="NovaCloud", sector=Sector.SAAS), review=[])
    monkeypatch.setattr("portfolio_extract.cli.extract_pdf", lambda p: de)
    monkeypatch.setenv("LLM_MODEL", "test-model")
    db = tmp_path / "out.db"
    run([str(data_dir / "NovaCloud_Q2_2025.pdf")], db_path=db)
    manifest = json.loads((tmp_path / "run_manifest.json").read_text())
    assert manifest["model"] == "test-model"
    assert manifest["records"] == 1 and manifest["documents"] == 1
    assert manifest["by_tier"]["HIGH"] == 1 and manifest["by_method"]["table_cell"] == 1
